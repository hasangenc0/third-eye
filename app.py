...
def predict():
    keys = ['manufacturer_name', 'model_name', 'transmission', 'color', 'odometer_value', 'year_produced',
            'engine_fuel', 'engine_has_gas', 'engine_capacity', 'body_type', 'has_warranty', 'drivetrain',
    ...
    knn = KNClassifier()
    knn.load_dataset('data/cars.csv', label=label, titles=True)
    knn.load_data_for_predict_as_list(data)
    predictions = knn.predict(k=5)
    result = ','.join(['{:.2f}'.format(x[0]) for x in predictions])
    return jsonify({'result': result, 'currency': 'US Dollar'})
...
class KNClassifier:
    ...
    def load_dataset(self, filename, label, batch=100, titles=False):
        with open(filename, 'rt') as csv_file:
            ...
            self.dataSet = [[] for _ in range(length)]
            self.label = label
